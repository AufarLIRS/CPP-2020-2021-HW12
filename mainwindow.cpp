...
void MainWindow::on_CalculatePriceButton_clicked()
{
  switch (Transport(ui->comboBox_2->currentIndex()))
  {
    case Transport::Air: {
      AirDelivery offer =
          AirDelivery(Continent(ui->comboBox->currentIndex()), Continent(ui->comboBox_3->currentIndex()));
      ui->price->setText(QString::number(offer.getPrice()));
      break;
    }
    case Transport::Ground: {
      GroundDelivery offer =
          GroundDelivery(Continent(ui->comboBox->currentIndex()), Continent(ui->comboBox_3->currentIndex()));
      ui->price->setText(QString::number(offer.getPrice()));
      break;
    }
    case Transport::Sea: {
      SeaDelivery offer =
          SeaDelivery(Continent(ui->comboBox->currentIndex()), Continent(ui->comboBox_3->currentIndex()));
      ui->price->setText(QString::number(offer.getPrice()));
      break;
    }
    default: {
